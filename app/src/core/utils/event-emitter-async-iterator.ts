...
export class EventEmitterAsyncIterator<T>
  implements AsyncIterableIterator<EventEmitterAsyncIteratorOutput<T>>
{
  ...
  async next(): Promise<IteratorResult<EventEmitterAsyncIteratorOutput<T>>> {
    if (this.eventQueue.length > 0) {
      ...
      return {
        value: value as EventEmitterAsyncIteratorOutput<T>,
        done: false,
      };
    } else {
  ...
  [Symbol.asyncIterator](): AsyncIterableIterator<
    EventEmitterAsyncIteratorOutput<T>
  > {
    return this;
  }
}
