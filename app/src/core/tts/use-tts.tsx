...
function extractTextSegments(element: HTMLElement) {
  const selector =
    'p, li, th, td, blockquote, pre code, h1, h2, h3, h3, h5, h6';
  const nodes = Array.from(element.querySelectorAll(selector) || []);
  const lines: string[] = [];
  const blocks = nodes.filter(
    (node) => !node.parentElement?.closest(selector) && node.textContent
  );
...
export function useTTSPlayerState(): ITTSContext {
  ...
  const ttsPluginImpl = useMemo(() => {
    const ttsPluginIndex =
      ttsPlugins.findIndex((p) => new p().describe().id === ttsPluginID) || 0;
  ...
  const setSourceElement = useCallback(
    (newKey: string | null, element: HTMLElement | null) => {
      ...
      if (element) {
        if (!plugin.current) {
          const pluginContext = createBasicPluginContext(
            ttsPluginID,
            context.chat.options,
            context.id,
...
export function TTSContextProvider(props: { children: React.ReactNode }) {
  ...
  return (
    <TTSContext.Provider value={context}>{props.children}</TTSContext.Provider>
  );
}
