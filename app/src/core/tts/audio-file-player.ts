...
export default class ExternalTTSAudioFilePlayer extends AbstractTTSPlayer {
  ...
  private download = async () => {
    ...
    const maxSentencesToDownload = this.sourceNodes[this.currentSentenceIndex]
      ? 2
      : 1;
  ...
  private schedule = async () => {
    ...
    try {
      for (
        let i = this.requestedSentenceIndex;
        i < this.sentences.length;
        i++
      ) {
        if (this.destroyed) {
          return;
        ...
        if (!this.sourceNodes[i]) {
          ...
          sourceNode.onended = async () => {
            ...
            const isBuffering =
              !this.ended && !this.sourceNodes[this.currentSentenceIndex];
  ...
  public getState(): TTSPlayerState {
    return {
      ...
      downloadable:
        this.complete && this.sourceNodes.length === this.sentences.length,
    } as any;
  }
