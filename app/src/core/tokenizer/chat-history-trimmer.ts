...
export class ChatHistoryTrimmer {
  ...
  public process() {
    ...
    this.output = [tokenizer.truncateMessage(last, this.options.maxTokens)];
  ...
  private removeUnwantedMessagesStrategy() {
    ...
    for (let i = 0; i < this.output.length; i++) {
      if (i === systemPromptIndex && this.options.preserveSystemPrompt) {
        ...
        i === firstUserMessageIndex &&
        this.options.preserveFirstUserMessage
      ) {
        output.push(this.output[i]);
  ...
  private removeMessagesStrategy() {
    ...
    for (
      let i = 0;
      i < this.output.length &&
      tokenizer.countTokensForMessages(output) > this.options.maxTokens;
      i++
    ) {
